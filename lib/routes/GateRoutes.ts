...
export class GateRoutes {
    ...
    public routes(app: Application): void {
        ...
        app.get("/gateProtectedAlive", this.isAuthenticated, asyncHandler(async (req: IRequest, res: Response, next: NextFunction) => {
            let message = await gateController.protectedAlive(req, res, next);
            res.send(message);
        }));

        app.post("/moveGate", this.isAuthenticated, exchangeTokenMiddleware,
        asyncHandler(async (req: IRequest, res: Response, next: NextFunction) => {
            let message = await gateController.moveGate(req, res, next);
            res.send(message);
        }));

        app.post("/openGate", this.isAuthenticated, asyncHandler(async (req: IRequest, res: Response, next: NextFunction) => {
            let message = await gateController.openGate(req, res, next);
            res.send(message);
        }));

        app.post("/closeGate", this.isAuthenticated, asyncHandler(async (req: IRequest, res: Response, next: NextFunction) => {
